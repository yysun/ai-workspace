...
const DEFAULT_SYSTEM_PROMPT = [
  ...
  "When the workspace config exposes `api_request`, prefer it for API calls scoped to the configured API_BASE_URL because auth and security headers are applied by the host. For large responses that you may need to inspect later, pass outputFilePath under the tool-owned api-responses directory so the body is saved to disk and the path is returned explicitly; automatic spills do not expose generated file paths. For repeatable GET requests, pass cacheTtlMs to enable user-scoped file-backed caching and bypassCache when you need a refresh.",

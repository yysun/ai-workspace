...
const DEFAULT_SYSTEM_PROMPT = [
  ...
  "When the workspace config exposes `api_request`, prefer it for API calls scoped to the configured API_BASE_URL because auth and security headers are applied by the host. Responses are returned inline by default; pass outputFilePath under the tool-owned api-responses directory only when you want the body saved to disk and the path returned explicitly. For repeatable GET requests, pass cacheTtlMs to enable in-memory caching and bypassCache when you need a refresh.",
